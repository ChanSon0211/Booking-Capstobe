          ...
          <Tabs
            ...
            items={
              movieDetail.heThongRapChieu.map(itemRap => {
                return {
                  label: <div> <img className="w-24" src={itemRap.logo} /> <br />{itemRap.tenHeThongRap} </div>,
                  key: itemRap.maHeThongRap,
                  children: itemRap.cumRapChieu.map(itemCumRap => {
                    return <p className="text-lg text-red-700">{itemCumRap.tenCumRap} ({itemCumRap.diaChi})
                   <p> {itemCumRap.lichChieuPhim.map(itemLichChieu => {
                      return <Tag>{itemLichChieu.tenRap} ({moment(itemLichChieu.ngayChieuGioChieu).format("DD-MM-YY hh:mm")})</Tag>
                    })}
                     </p></p>
